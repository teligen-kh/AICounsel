...
class SafeCSVImporter:
    ...
    def load_csv_safe(self, csv_file_path: str, encoding: str = 'utf-8') -> pd.DataFrame:
        """안전한 CSV 로드 (쉼표 이슈 해결)"""
        try:
            logger.info(f"안전한 CSV 파일 로드 중: {csv_file_path}")
            
            # 방법 1: pandas with quoting
            try:
                df = pd.read_csv(
                    csv_file_path, 
                    encoding=encoding,
                ...
                logger.info(f"pandas로 로드 완료: {len(df)}행")
                return df
            except Exception as e:
                logger.warning(f"pandas 로드 실패: {str(e)}")
            
            # 방법 2: 직접 CSV 파싱
            logger.info("직접 CSV 파싱 시도...")
            data = []
                ...
                for row_num, row in enumerate(reader, start=2):
                    try:
                        ...
                        if len(row) != len(headers):
                            logger.warning(f"행 {row_num}: 셀 수 불일치 (예상: {len(headers)}, 실제: {len(row)})")
                            continue
                        ...
                        logger.warning(f"행 {row_num} 처리 중 오류: {str(e)}")
                        continue
            
            df = pd.DataFrame(data, columns=headers)
            logger.info(f"직접 파싱 완료: {len(df)}행")
            return df
...
async def main():
    ...
    try:
        importer = SafeCSVImporter(db)
        
        # CSV 파일 경로 (구글 스프레드시트에서 다운로드한 파일)
        csv_file_path = "D:/AICounsel/data/csv/knowledge_base_ai_counseling_data.csv"
        ...
        # 안전한 CSV 로드
        df = importer.load_csv_safe(csv_file_path)
